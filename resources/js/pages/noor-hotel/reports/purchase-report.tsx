...
type Summary = {
    ...
    total_vat: number;
    total_amount: number;
    total_delivered_subtotal: number;
    total_delivered_vat: number;
    total_delivered_amount: number;
    total_remaining_subtotal: number;
    total_remaining_vat: number;
...
export default function PurchaseReport({ parties }: { parties: Party[] }) {
    ...
    const printReport = () => {
        ...
        const dataRows = rows
            .map(
                (r, i) => `<tr>
                    ...
                    <td style="padding:3px 6px;border:1px solid #000;font-size:10px;text-align:right;">Tk ${fmt$(r.delivered_total)}</td>
                    <td style="padding:3px 6px;border:1px solid #000;font-size:10px;text-align:right;">Tk ${fmt$(r.remaining_total)}</td>
        ...
        const partialTotal = partialRows.reduce((s, r) => s + r.delivered_total, 0);
        ...
        const partialDeliveredAmt = partialRows.reduce((s, r) => s + r.delivered_total, 0);
        const partialRemainingAmt = partialRows.reduce((s, r) => s + r.remaining_total, 0);
        const partialOrderedAmt = partialRows.reduce((s, r) => s + r.total, 0);
        ...
        <table>
            <thead>
                <tr>
                    <th colspan="16" style="padding:4px 6px;border:none;text-align:center;background:#fff;">
                ...
                <tr style="background:#f1f5f9;">
                    ...
                    <th style="padding:4px 6px;font-size:10px;text-align:right;width:76px;">Delivered Amt</th>
                    <th style="padding:4px 6px;font-size:10px;text-align:right;width:76px;">Remaining Amt</th>
                    ...
                    <table style="width:100%;border-collapse:collapse;">
                        ...
                        <tr>
                            <td style="padding:3px 6px;border:1px solid #000;font-size:10px;background:#fee2e2;">
                                Delivered: ${fmtNum(partialDeliveredQty)} packs (Tk ${fmt$(partialDeliveredAmt)}) | Remaining: ${fmtNum(partialRemainingQty)} packs (Tk ${fmt$(partialRemainingAmt)})
                            </td>
                            <td style="padding:3px 6px;border:1px solid #000;font-size:10px;text-align:right;background:#fee2e2;">
                                Ordered: ${fmtNum(partialOrderedQty)} packs (Tk ${fmt$(partialOrderedAmt)})
                            </td>
                        </tr>
                        <tr>
                ...
                {summary && (
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-9">
                        {[
                            ...
                            { label: 'Delivered Amt', value: `Tk ${fmt$(summary.total_delivered_amount)}`, icon: Truck, color: 'text-emerald-600' },
                            { label: 'Remaining Amt', value: `Tk ${fmt$(summary.total_remaining_amount)}`, icon: AlertCircle, color: 'text-red-600' },
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-sidebar-border/70 bg-muted/50 dark:border-sidebar-border">
                                    ...
                                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Delivered Amt</th>
                                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Remaining Amt</th>
                                    ...
                                    rows.map((r) => {
                                        ...
                                        return (
                                            <tr key={r.id} className="border-b border-sidebar-border/70 transition-colors last:border-0 hover:bg-muted/30 dark:border-sidebar-border">
                                                ...
                                                <td className="px-4 py-3 text-right text-xs font-medium tabular-nums text-emerald-600">Tk {fmt$(r.delivered_total)}</td>
                                                <td className="px-4 py-3 text-right text-xs tabular-nums text-red-600">Tk {fmt$(r.remaining_total)}</td>
